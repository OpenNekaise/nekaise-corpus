---
name: crawl-docs
description: Add a multi-page documentation site (software/ontology docs like EnergyPlus, VOLTTRON, Brick, Haystack, ASHRAE 223P) to the corpus. Use when a valuable reference is a doc site, not a single PDF, and the single-URL loader can't reach it.
---
...
2. **Crawl + register** (needs network; run outside a sandbox):
   ```
   python scripts/crawl_docs.py --seed <url> --prefix <path> --source <tag> \
