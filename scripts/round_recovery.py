...
ownership (the caller's writer), stop the runs' processes (scoped to their coordinator attempt:
scripts/run_ownership.py — the legacy matching above is not used there, nor changed), drain the
broker, then the database decides: a promoted run stands (even after a lost reply) and its
completion is finished, an unpromoted one is aborted (resuming is explicit: run_round.py
--resume), an unknown outcome raises before anything is mutated; temporaries are swept.
"""
...
def recover_staged(st, writer, run_id: str | None = None, *, root: Path,
                   ...
                   stop: bool = True, finish: bool = True, owner=None) -> list[StagedOutcome]:
    ...
    2. stop their processes — the caller's new descendants and every process of the run's
       coordinator attempt (scripts/run_ownership.py: `owner`, the caller's own attempt, or the
       one the run's ownership mark records; a mark whose coordinator is still alive and is
       not the caller refuses) — unless `stop` is False (the caller already did). Every stop is
       confirmed (no survivor) or raises before anything is decided;
    ...
    try:
        if stop:
            ...
            for rid in targets:
                stopped[rid] = run_ownership.stop_attempt(
                    root, rid, existing=owned, grace=grace,
                    current=owner if owner is not None and owner.run_id == rid else None)
            ...
            if not targets and owned is not None:
                run_ownership.stop(descendants(os.getpid()) - owned, None, grace)
    finally:
    ...
    for rid in targets:
        ...
        if stop and owner is None:   # its dead attempt's processes are confirmed stopped
            rec = run_ownership.read_mark(root, rid)
