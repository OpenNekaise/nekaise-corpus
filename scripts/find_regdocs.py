...
def runnable(sources: dict[str, dict]) -> list[str]:
    return [k for k, c in sources.items() if c.get("enabled") and not c.get("blocked")]
...
def make_entry(key: str, cfg: dict, url: str, title: str, today: str,
    ...
    title = base[:max(20, TITLE_MAX - len(suffix))].rstrip() + suffix
    entry = {
...
def universe(key: str, cfg: dict, budget: Budget, today: str, fetch=None) -> list[dict]:
    ...
    if mech == "static_list":
        for it in cfg["items"]:
            extra = {k: v for k, v in it.items() if k not in ("url", "title")}
            out[it["url"]] = make_entry(key, cfg, it["url"], it["title"], today, extra)
    elif mech == "sitemap_pages":
        include = re.compile(cfg["include"])
        exclude = re.compile(cfg["exclude"]) if cfg.get("exclude") else None
        pending = list(cfg["sitemaps"])
        children = 0
        while pending:
            sm = pending.pop(0)
            budget.take(sm)
...
def fingerprint(entries: list[dict]) -> str:
    return hashlib.sha1("\n".join(e["url"] for e in entries[:16]).encode()).hexdigest()[:10]
