...
digging for more. This is what a new user does after cloning and opening Claude Code: just say
**"go"**.
...
python build_corpus.py               # fetch every missing source from sources.yaml
```
...
[`dig`](dig.md) skill — the cron just runs it headless.

If **no**: leave it off; mention they can enable it later with `go` or `bash scripts/install_cron.sh`,
or grow the corpus by hand anytime with the [`dig`](dig.md) skill (one full round), or the individual
[`find-sources`](find-sources.md) / [`crawl-docs`](crawl-docs.md) skills and `python find_github.py`.
